...
class LogisticRegression(object):
    ...
    def sigmoid(self, s: np.ndarray) -> np.ndarray: # [2pts]
        ...
        """
        np_array = np.empty(s.shape)
        np_array = 1/(1 + np.exp(-s))
        return np_array
    ...
    def bias_augment(self, x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        """
        x_aug = np.empty((x.shape[0], x.shape[1] + 1))
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float: # [3pts]
        ...
        """
        sleft = y * np.log(h_x)
        sright = (1 - y) * np.log(1 - h_x)
        return (-np.mean(sleft + sright))
    ...
    def evaluate(
        ...
        predict_probs = self.predict_probs(self.bias_augment(x), theta)
    ...
    def fit(
        ...
        for i in range(epochs):
            predict_probs = self.predict_probs(x_augment, theta)
            gradient = self.gradient(x_augment,y_train,predict_probs)
            theta -= gradient*lr
            if i % 100 == 0:
